...
-- antall saker totalt i arena-historikken
select count(distinct sak_id) as totalt_tall from vedtak;

-- antall signifikante saker med denne utgaven av filter-spørringer
select count(distinct sak_id) as nyeste_tall
from (SELECT sak_id,
             vedtakstatuskode,
             vedtaktypekode,
             fra_dato,
      ...
      WHERE (v.utfallkode IS NULL OR v.utfallkode != 'AVBRUTT')
        ...
        AND v.MOD_DATO >= DATE '2020-08-18'                                                   -- ytelse: unngå å løpe gjennom veldig gamle vedtak
        ...
        AND (
          (vedtaktypekode IN ('O', 'E', 'G') AND
           (til_dato IS NULL OR til_dato >= DATE '2025-02-18')) -- vanlig tidsbuffer
              OR
          (vedtaktypekode = 'S' AND NOT EXISTS(select vedtak_id
                                               from vedtak vv
                                               where vv.lopenrvedtak > v.lopenrvedtak
                                                 and vv.vedtak_id = v.vedtak_id_relatert
                                                 and vv.vedtaktypekode != 'S')
              AND
           (fra_dato IS NULL OR fra_dato >= DATE '2024-05-07')) -- ekstra tidsbuffer for Stans, som bare har fra_dato
          )
        AND NOT (utfallkode = 'NEI' AND til_dato IS NULL AND
                 (fra_dato IS NOT NULL AND fra_dato <= DATE '2025-02-18'))                    -- utfallkode NEI vil ha åpen til_dato, så ekskluder disse når de er gamle
      UNION ALL
      SELECT sak_id,
             vedtakstatuskode,
             vedtaktypekode,
...
-- Dersom den er null, er klagen fortsatt under behandling.
      ...
      WHERE (v.utfallkode IS NULL OR v.utfallkode != 'AVBRUTT')
        ...
        AND (vf.vedtakverdi IS NULL OR TO_DATE(vf.vedtakverdi, 'DD-MM-YYYY') >= DATE '2025-02-18')
        -- Dersom klagen ble innvilget for mer enn 6 mnd siden, regnes den som ikke relevant lenger. Ekskluder disse.
        AND NOT (vf.vedtakverdi IS NOT NULL AND TO_DATE(vf.vedtakverdi, 'DD-MM-YYYY') <= DATE '2026-02-18' AND
