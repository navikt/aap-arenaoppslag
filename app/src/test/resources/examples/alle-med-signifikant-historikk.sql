...
from (SELECT sak_id,
      ...
      WHERE (v.utfallkode IS NULL OR v.utfallkode != 'AVBRUTT')
        ...
        AND ((fra_dato IS NOT NULL OR til_dato IS NOT NULL) OR
             vedtakstatuskode IN ('OPPRE', 'MOTAT', 'REGIS',
                                  'INNST'))                                               -- filtrer ut etterregistrerte vedtak, men behold vedtak som er under behandling
        AND (
          (vedtaktypekode IN ('O', 'E', 'G') AND
           (til_dato IS NULL OR til_dato >= DATE '2024-12-17')) -- vanlig tidsbuffer på 18 måneder
              OR
          (vedtaktypekode = 'S' AND NOT EXISTS(select vedtak_id from vedtak vv where
              vv.lopenrvedtak > v.lopenrvedtak and vv.vedtak_id=v.vedtak_id_relatert and vv.vedtaktypekode !='S') AND
           (fra_dato IS NULL OR fra_dato >= DATE '2024-03-04')) -- ekstra tidsbuffer for Stans, som bare har fra_dato
          )
        AND NOT (utfallkode = 'NEI' AND til_dato IS NULL AND
                 (fra_dato IS NOT NULL AND fra_dato <= DATE '2024-12-17'))                -- utfallkode NEI vil ha åpen til_dato, så ekskluder disse når de er gamle
      UNION ALL
      SELECT sak_id,
             vedtakstatuskode,
             vedtaktypekode,
...
-- Dersom den er null, er klagen fortsatt under behandling.
      ...
      WHERE (v.utfallkode IS NULL OR v.utfallkode != 'AVBRUTT')
        ...
        AND (vf.vedtakverdi IS NULL OR
             TO_DATE(vf.vedtakverdi, 'DD-MM-YYYY') >= DATE '2024-12-17')
        ...
        AND NOT (vf.vedtakverdi IS NOT NULL AND
                 TO_DATE(vf.vedtakverdi, 'DD-MM-YYYY') <=
                 DATE '2025-12-15' AND v.utfallkode IN ('JA', 'DELVIS'))
      UNION ALL
      SELECT v.sak_id,
             vedtakstatuskode,
             vedtaktypekode,
