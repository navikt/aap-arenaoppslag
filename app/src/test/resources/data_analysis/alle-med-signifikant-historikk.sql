-- Spørring for å hente utdrag av alle vedtak som regnes som signifikant historikk

-- Kreves i arena-q1:
-- alter session set current_schema = "ARENA_TILGANG_AAP";

SELECT vedtak.person_id,
       vedtak.aar,
       vedtak.lopenrvedtak,
       vedtak.lopenrsak,
...
FROM (
      ...
      WHERE (v.utfallkode IS NULL OR v.utfallkode != 'AVBRUTT')
        ...
        AND NOT (utfallkode = 'NEI' AND til_dato IS NULL AND
                 (fra_dato IS NOT NULL AND fra_dato <= DATE '2024-06-17')
            )                    -- utfallkode NEI vil ha åpen til_dato, så ekskluder disse når de er gamle
)
      UNION ALL
      SELECT person_id,
             aar,
      ...
      WHERE (v.utfallkode IS NULL OR v.utfallkode != 'AVBRUTT')
        ...
        AND v.MOD_DATO >= DATE '2019-12-15' -- ytelse: unngå å løpe gjennom veldig gamle vedtak
         ...
         JOIN sak ON sak.sak_id = vedtak.sak_id
